...
if($schValue != ""){
	if($schField == ""){
		$sch_sql = " AND ( user_id LIKE '%".$schValue."%' OR user_name LIKE '%".$schValue."%' ) ";
	}else{
...
$sql = "SELECT COUNT(*) FROM t_member WHERE level in (1, 2)".$sch_sql;
...
<?
					if($TotalCnt > 0){
						if($page > ceil($TotalCnt / $size)) {
							$page = ceil($TotalCnt / $size);
						}
					
						$start = ($page-1) * $size;
						if($stateYn=="Y" or $stateYn=="N") {
							$dumpCnt = mysql_result($result , 0 ,0 );
							$count = $dumpCnt - (($page-1) * $size);
						} else {
						...
						$sql = "SELECT * FROM `t_member` WHERE level in (1, 2)".$sch_sql." ORDER BY user_regist_day DESC LIMIT ".$start.",".$size;
